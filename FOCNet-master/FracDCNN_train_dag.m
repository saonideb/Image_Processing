...
opts.batchSize = 128 ;
opts.gpus = [];
opts.numEpochs = 500 ; %no of epoch
...
switch CurTask
    case 'Denoising'
            %input = test_code(label);% add AWGN with noise level noiselevel
            input      = label + noiselevel/255*randn(size(label),'single');

    case 'SISR' % Single Image super-resolution
        for ii = 1:numel(batch)
            %input(:,:,:,ii) = test_code(label(:,:,:,ii));
            input(:,:,:,ii) = imresize(label(:,:,:,ii), 1/noiselevel,'bicubic');
        end
end
...
%{
-------sz is the learnable paramerter --params.learningRate or thisLR
